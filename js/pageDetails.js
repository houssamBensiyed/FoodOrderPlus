...
(function () {
  ...
  // 2) Base used to resolve "./images/..." from the JSON
  const DATA_BASE = new URL(".", DATA_URL); // -> https://younesbahmoun.github.io/food-order-data/
  ...
  // Resolve "./images/..." **relative to the JSON site**
  const resolveImage = (p) => {
    if (!p) return PLACEHOLDER;
    if (/^https?:\/\//i.test(p)) return p; // absolute already
    return new URL(p.replace(/^\.\//, ""), DATA_BASE).href; // ./images/x.png -> https://.../images/x.png
  };

  // Always attach onerror BEFORE src; add a tiny cache-bust so swaps actually reload
  ...
  // Quick random sampler (exclude a specific id if provided)
  ...
  async function init() {
    ...
    // Pick product by ?id=..., otherwise default to first
    const product = id
    ...
    if (product) {
      ...
      // Thumbnails: related (same category) else random
    ...
    // Fill the 7-tile grid with random items (excluding current product if any)
